...
class Qwen3TTSTokenizerV2ConvNeXtBlockUnfused(nn.Module):
    """Original unfused block — used only for loading pretrained weights."""
    def __init__(self, dim: int):
        ...
        self.norm = nn.LayerNorm(dim, eps=1e-6)  # elementwise_affine=True by default
...
class Qwen3TTSTokenizerV2ConvNeXtBlock(nn.Module):
    """Fused block — norm affine absorbed into pwconv1, gamma absorbed into pwconv2."""
    def __init__(self, dim: int):
        super().__init__()
        self.dwconv = Qwen3TTSTokenizerV2CausalConvNet(
            dim,
    ...
    @staticmethod
    def from_unfused(unfused: "Qwen3TTSTokenizerV2ConvNeXtBlockUnfused") -> "Qwen3TTSTokenizerV2ConvNeXtBlock":
        """Create a fused block from an unfused one."""
        ...
        # Fuse LayerNorm affine into pwconv1
...
class SnakeBeta(nn.Module):
    ...
    def __init__(self, in_features, alpha=1.0):
        ...
        # Precomputed values (populated by calling precompute())
        self._alpha_exp: Optional[torch.Tensor] = None
        self._beta_inv: Optional[torch.Tensor] = None

    def precompute(self) -> None:
        """Precompute exp(alpha) and 1/exp(beta) for faster inference."""
    ...
    def forward(self, hidden_states):
        ...
        if self._alpha_exp is not None and self._beta_inv is not None:
            ...
            beta_inv = 1.0 / (torch.exp(beta) + self.no_div_by_zero)

        hidden_states = hidden_states + beta_inv * torch.pow(
            torch.sin(hidden_states * alpha_exp), 2
        )
...
class EuclideanCodebook(nn.Module):
    ...
    def precompute_embedding(self) -> None:
        """Precompute the normalized embedding lookup table."""
        self._embedding = self.embedding_sum / self.cluster_usage.clamp(min=self.epsilon)[:, None]

    def decode(self, codes: torch.Tensor) -> torch.Tensor:
        if self._embedding is None:
            self.precompute_embedding()
        quantized = F.embedding(codes, self._embedding)
        return quantized
...
class Qwen3TTSTokenizerV2Model(Qwen3TTSTokenizerV2PreTrainedModel):
    ...
    def encode(
        ...
        for code, mask in zip(audio_codes, padding_mask):
            # Calculate effective code length using ceiling division: ceil(mask.sum() / rate)
            valid_code_len = -(-mask.sum() // self.encode_downsample_rate)
            processed_codes.append(code[..., :valid_code_len].transpose(0, 1))
        audio_codes = processed_codes
        if not return_dict:
            return (
                audio_codes,
            )
