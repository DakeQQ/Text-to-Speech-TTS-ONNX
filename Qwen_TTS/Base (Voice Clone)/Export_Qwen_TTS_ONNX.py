...
class TTS_DECODER(torch.nn.Module):
    ...
    Combines reference codec tokens with the generated sequence,
    then runs the Mimi decoder and optional output resampling.
    """

    def __init__(self, tts, output_sample_rate, max_seq_len):
        super().__init__()
        self.tts = tts
        self._replace_gelu_with_tanh_approximation(self.tts.model)
    ...
    def forward(self, ref_code, ref_code_len, generated_codec):
...
if DO_EXPORT:
    ...
    with torch.inference_mode():
        ...
        decoder = TTS_DECODER(model, OUT_SAMPLE_RATE, MAX_SEQ_LEN)
        del model
        gc.collect()
        torch.onnx.export(
            decoder,
