...
class MimiConv1d(nn.Module):
    ...
    def forward(self, hidden_states, padding_cache=None):
        ...
        if self.causal and padding_cache is not None:
            ...
            hidden_states = self._pad1d(hidden_states, (self.padding_total, 0), mode=self.pad_mode)
...
class MimiEuclideanCodebook(nn.Module):
    ...
    def embed(self) -> torch.Tensor:
        if self._embed is None:
            self._embed = self.embed_sum / self.cluster_usage.unsqueeze(-1)
        return self._embed

    @property
    def embed_T(self) -> torch.Tensor:
        if self._embed_T is None:
            self._embed_T = self.embed_sum.t() / self.cluster_usage
        return self._embed_T

    @property
    def embed_norm(self) -> torch.Tensor:
        if self._embed_norm is None:
            # Pre-compute the squared norm of codebook vectors: ||y||^2
            # Shape: [1, CodebookSize] to allow broadcasting
            self._embed_norm = (self.embed_T * self.embed_T).sum(0, keepdim=True)
        return self._embed_norm
    ...
    def quantize(self, hidden_states):
        ...
        # or [B, T, D] depending on the caller.

        # Replaces torch.cdist with ONNX-friendly ops.
        # We find argmin( ||x - y||^2 )
        # <=> argmin( ||x||^2 - 2<x, y> + ||y||^2 )
        # <=> argmin( -2<x, y> + ||y||^2 )  since ||x||^2 is constant w.r.t y

        # 1. Compute dot product <x, y>: hidden_states @ embed.T
        # Shape: [..., codebook_size]
        dot_product = torch.matmul(hidden_states, self.embed_T)

        # 2. Compute proxy distance: ||y||^2 - 2<x, y>
        # We reuse the pre-computed embedding norms.
        dist = self.embed_norm - (dot_product + dot_product)

        embed_ind = dist.argmin(dim=-1).int()
        return embed_ind
