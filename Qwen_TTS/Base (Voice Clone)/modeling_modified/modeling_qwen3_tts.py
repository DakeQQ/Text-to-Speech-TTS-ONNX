...
class AttentiveStatisticsPooling(nn.Module):
    ...
    def _compute_statistics(self, x, m, dim=2):
        ...
        temp = x - mean
        std = torch.sqrt((m * (temp * temp)).sum(dim, keepdim=True))
        return mean, std

    def forward(self, hidden_states):
        seq_length = hidden_states.shape[-1].unsqueeze(0)
        mean, std = self._compute_statistics(hidden_states, 1.0 / seq_length)
        mean = mean.repeat(1, 1, seq_length)
        std = std.repeat(1, 1, seq_length)
...
class Qwen3TTSForConditionalGeneration(Qwen3TTSPreTrainedModel, GenerationMixin):
    ...
    def extract_speaker_embedding(self, audio, sr):
        ...
        mels = mel_spectrogram(
            ...
            fmin=0, 
            fmax=12000
        ).transpose(1, 2).to(self.device).to(self.dtype)
