...
class AMPBlock1(torch.nn.Module):
    ...
    def forward(self, x, x_shape, up_filter_pad, up_pad_zeros, down_filter_pad, down_pad_zeros_L, down_pad_zeros_R):
        ...
        for c1, c2, a1, a2 in zip(self.convs1, self.convs2, acts1, acts2):
            xt = a1(x, x_shape, up_filter_pad, up_pad_zeros, down_filter_pad, down_pad_zeros_L, down_pad_zeros_R)
            xt = c1(xt)
            xt = a2(xt, x_shape, up_filter_pad, up_pad_zeros, down_filter_pad, down_pad_zeros_L, down_pad_zeros_R)
            xt = c2(xt)
            x = xt + x
        return x
...
class BigVGAN(
    ...
    def __init__(self, h: AttrDict, use_cuda_kernel: bool = False):
        ...
        up_filter = self.activation_post.upsample.filter
        self.x_shape = []
        self.up_filter_pad = []
        self.up_pad_zeros = []
        ...
        self.down_pad_zeros_L = []
        self.down_pad_zeros_R = []
        for i in range(self.num_upsamples):
            shape = self.ups._modules[f"{i}"]._modules["0"].out_channels
    ...
    def forward(self, x):
        ...
        for i in range(self.num_upsamples):
            ...
            for i_up in range(len(self.ups[i])):
                x = self.ups[i][i_up](x)
            # AMP blocks
            xs = self.resblocks[i * self.num_kernels](x, self.x_shape[i], self.up_filter_pad[i], self.up_pad_zeros[i], self.down_filter_pad[i], self.down_pad_zeros_L[i], self.down_pad_zeros_R[i])
            for j in range(1, self.num_kernels):
                if xs is None:
                    xs = self.resblocks[i * self.num_kernels + j](x, self.x_shape[i], self.up_filter_pad[i], self.up_pad_zeros[i], self.down_filter_pad[i], self.down_pad_zeros_L[i], self.down_pad_zeros_R[i])
                else:
                    xs += self.resblocks[i * self.num_kernels + j](x, self.x_shape[i], self.up_filter_pad[i], self.up_pad_zeros[i], self.down_filter_pad[i], self.down_pad_zeros_L[i], self.down_pad_zeros_R[i])
            x = xs * self.inv_num_kernels

        # Post-conv
        x = self.activation_post(x, self.x_shape[-1], self.up_filter_pad[-1], self.up_pad_zeros[-1], self.down_filter_pad[-1], self.down_pad_zeros_L[-1], self.down_pad_zeros_R[-1])
        x = self.conv_post(x)
