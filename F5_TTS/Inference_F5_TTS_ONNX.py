...
# ============================== Configuration ==============================
# Edit runtime values directly; the CLI is reserved for model asset paths.
...
if TEST_IN_ENGLISH:
    REFERENCE_AUDIO_PATH = Path(python_package_path) / "f5_tts/infer/examples/basic/basic_ref_en.wav"
    REFERENCE_TEXT = "Some call me nature, others call me mother nature."
    TARGET_TEXT = "Some call me Dake, others call me QQ."
else:
    REFERENCE_AUDIO_PATH = Path(python_package_path) / "f5_tts/infer/examples/basic/basic_ref_zh.wav"
    REFERENCE_TEXT = "对，这就是我，万人敬仰的太乙真人。"
    TARGET_TEXT = "对，这就是我，万人敬仰的大可奇奇。"

ORT_Accelerate_Providers = []  # Optional providers, for example ['CUDAExecutionProvider'].
RANDOM_SEED = 9527             # Seed used to reproduce generated audio.
...
onnx_model_Metadata    = str(onnx_folder / "F5_Metadata.onnx")
generated_audio = str(GENERATED_AUDIO_PATH.expanduser().resolve())
reference_audio = str(REFERENCE_AUDIO_PATH.expanduser().resolve())
...
NFE_STEP          = int(model_meta["nfe_step"])
MAX_SIGNAL_LENGTH = int(model_meta["max_signal_length"])
onnx_model_Preprocess = str(onnx_folder / model_meta["model_file_name_preprocess"])
onnx_model_Transformer = str(onnx_folder / model_meta["model_file_name_transformer"])
onnx_model_Decode = str(onnx_folder / model_meta["model_file_name_decode"])
...
print_progress(
    f"Metadata ready: input={IN_SAMPLE_RATE} Hz,"
