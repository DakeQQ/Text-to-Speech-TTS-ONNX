...
class STFT_Process(torch.nn.Module):
    ...
    def __init__(
        ...
        window = create_padded_window(
            win_length,
            self.analysis_length if model_type.startswith('stft_') else n_fft,
            window_type,
        )
    ...
    def _build_stft_kernels(
        ...
        t = torch.arange(self.analysis_length, dtype=torch.float64).unsqueeze(0)
        f = torch.arange(f_bins, dtype=torch.float64).unsqueeze(1)
        omega = omega_factor * f * t
        window = window.to(torch.float64)
        ...
        if model_type == 'stft_A':
            ...
            kernel = torch.cat([windowed_cos, windowed_sin], dim=0)

        if remove_dc_offset or preemphasis:
            transform = torch.eye(self.analysis_length, dtype=torch.float64)
            if remove_dc_offset:
                transform -= torch.full_like(
                    transform,
                    1.0 / self.analysis_length,
                )
            if preemphasis:
                preemphasis_matrix = torch.eye(
                    self.analysis_length,
                    dtype=torch.float64,
                )
                preemphasis_matrix[0, 0] = 1.0 - preemphasis
                preemphasis_matrix.diagonal(offset=-1).fill_(-preemphasis)
                transform = preemphasis_matrix @ transform
            kernel = torch.matmul(kernel.squeeze(1), transform).unsqueeze(1)
        self.register_buffer('stft_kernel', kernel.float().contiguous())
