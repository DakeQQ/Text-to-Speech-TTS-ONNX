...
# User configuration
home_path = Path.home()
project_path = home_path / "Downloads" / "index-tts-main"       # IndexTTS project: https://github.com/index-tts/index-tts
models_path = home_path / "Downloads" / "IndexTTS-2"            # Model folder: https://modelscope.cn/models/IndexTeam/IndexTTS-2/files
emotion_text_model_path = models_path / "qwen0.6bemo4-merge"

MAX_SIGNAL_LENGTH = 2048
USE_F16_KV = True
COMPUTE_IN_F32 = False
OPSET = 20
CFM_STEPS = 25                            # Fixed at export time
...
EMOTION_TEXT_REORDER_DOWNPROJ = True      # Exact MLP channel reorder for Q4/Q8 weights.
EMOTION_TEXT_REORDER_KEY = "absmean"      # absmean | L4 | rms | std
EMOTION_TEXT_KV_DTYPE = "F16"             # F16 | F32
...
class IndexTTS2Main(nn.Module):
    ...
    def __init__(self, gpt: nn.Module, max_sequence_length: int) -> None:
        ...
        self.register_buffer(
            "attention_mask",
            (
                (1 - torch.tril(torch.ones(1, 1, max_sequence_length, max_sequence_length, dtype=torch.int8)))
                * -128
            ),
        )
    ...
    def forward(self, *all_inputs: torch.Tensor) -> tuple[torch.Tensor, ...]:
        ...
        for index, layer in enumerate(self.inference_model.transformer.h):
            ...
            if USE_F16_KV and self.compute_in_f32:
                attention_output = torch.matmul(query, key.float()) + attention_mask
                attention_output = torch.matmul(
                    torch.softmax(attention_output, dim=-1),
                    value.float(),
                )
            else:
                attention_output = torch.matmul(query, key) + attention_mask
                attention_output = torch.matmul(
                    torch.softmax(attention_output, dim=-1),
                    value,
                )
...
class IndexTTS2AcousticConditioning(nn.Module):
    ...
    def forward(
        ...
        # Official inference is batch-one and uses unsqueeze(1), yielding the
        # quantizer-major [1, 1, time] layout expected by ResidualVQ.vq2emb.
    ...
    def __init__(self, estimator: nn.Module) -> None:
        ...
        with torch.inference_mode():
            step_times = torch.arange(
                CFM_STEPS,
                dtype=torch.float32,
                device=self.estimator.input_pos.device,
            ) / CFM_STEPS
            ...
            time_span = torch.arange(
                CFM_STEPS + 1,
                dtype=torch.float32,
                device=step_times.device,
            ) / CFM_STEPS
            euler_deltas = (time_span[1:] - time_span[:-1]).view(
                CFM_STEPS,
                1,
                1,
            )
...
def export_gpt_graphs(config: Any) -> None:
    ...
    conditioning = IndexTTS2Conditioning(
        gpt,
        speaker_matrix,
        emotion_matrix,
    )
    ...
    speaker_latent = torch.zeros(1, 32, hidden_size)
    emotion_vector = torch.zeros(1, hidden_size)
    text_ids = torch.tensor([[2, 3, 4, 5]], dtype=torch.int32)
    target = IndexTTS2TargetPreprocess(gpt)
    export_onnx(
        target,
        (speaker_latent, emotion_vector, text_ids),
        onnx_models["target"],
        input_names=["speaker_latent", "emotion_vector", "text_ids"],
    ...
    latent = IndexTTS2Latent(gpt, main_core)
...
def export_acoustic_graphs(
    ...
    mel_codes = torch.zeros(1, 20, dtype=torch.int32)
    gpt_latent = torch.zeros(1, 20, int(config.gpt.model_dim))
    ...
    null_hidden = cfm_projection.null_hidden
    export_onnx(
        acoustic,
        (mel_codes, gpt_latent, cfg_rate, style, reference_hidden, null_hidden),
    ...
    cfm_step = IndexTTS2CFMStep(cfm_estimator)
...
def main() -> None:
    ...
    auxiliary_paths = resolve_auxiliary_paths(config)
