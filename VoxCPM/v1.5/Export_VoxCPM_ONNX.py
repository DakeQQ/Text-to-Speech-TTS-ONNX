...
class VOXCPM_VAE_ENCODER(torch.nn.Module):
    def __init__(self, voxcpm, in_sample_rate):
        ...
        self.pad_zeros_right = torch.zeros([1, 1, self.patch_len], dtype=torch.float32)
    ...
    def forward(self, prompt_audio):
        ...
        prompt_audio = torch.cat([self.pad_zeros[..., :padding_size].float(), prompt_audio, self.pad_zeros_right], dim=-1)
